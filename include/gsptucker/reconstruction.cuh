...
void Reconstruction(TensorType* tensor, TensorType* core_tensor,
  ...
  uint64_t recon_queue_size = 10;

  std::vector<std::unique_ptr<queue_ctx_t>> recon_queues;
  recon_queues.reserve(device_count);

  for (int d = 0; d < device_count; ++d)
    recon_queues.emplace_back(new queue_ctx_t(recon_queue_size));
  ...
  std::thread producer_thread([&]() {
    for (const auto& task : tasks) {
        ...
        while (!enqueued) {
          ...
          for (int d = 0; d < device_count; ++d) {
            uint64_t load =
                recon_queues[d]->pending_nnz.load(std::memory_order_relaxed);
            if (load < min_load) {
              min_load = load;
              target_device = d;
            }
          ...
          auto& queue_ctx = *recon_queues[target_device];
          {
    ...
    for (auto& queue_ctx_ptr : recon_queues) {
...
#pragma omp parallel num_threads(device_count)
  {
    ...
    task_t queue_task{};
    auto& queue_ctx = *recon_queues[device_id];
