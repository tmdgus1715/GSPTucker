...
#pragma omp parallel num_threads(device_count)
  {
    ...
    for (const auto& task : tasks) {
      for (int part_id = 0; part_id < part_count_int; ++part_id) {
        if (tensor->blocks[task.block_id]->get_block_coord()[curr_factor_id] != part_id) continue;
      ...
      tensor_manager->GetDeltaChunk(block_id);
      ...
      // For Delta
      uint64_t delta_offset = task.delta_offset;
