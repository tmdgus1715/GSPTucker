...
void ComputingSubBC(TensorType* tensor, DeltaType* delta, MatrixType* B,
  ...
  // Helper lambda to process a block (parent or child)
...
void ComputingDeltaBC(TensorType* tensor, TensorType* core_tensor,
  ...
  std::vector<std::atomic<uint32_t>> blocks_remaining_per_part(
      static_cast<size_t>(part_count));
  for (uint64_t block_id = 0; block_id < block_count; ++block_id) {
    if (block_task_counts[block_id] > 0) {
      int p = tensor->blocks[block_id]->get_block_coord()[curr_factor_id];
      blocks_remaining_per_part[p].fetch_add(1, std::memory_order_relaxed);
    }
  }
...
#pragma omp parallel num_threads(device_count)
  {
    ...
    while (true) {
      ...
      if (prev == 1) {
        ...
        common::cuda::stream_synchronize(streams[stream_offset]);

        ComputingSubBC(tensor, delta, B[block_part], C[block_part], curr_factor_id, rank, block_id);

        uint32_t prev_blocks = blocks_remaining_per_part[block_part].fetch_sub(1, std::memory_order_acq_rel);
        if (prev_blocks == 1) {
          UpdateSubFactorMatrix<TensorType, MatrixType>(
              factor_matrices, B[block_part], C[block_part], curr_factor_id, block_part, row_count, rank);
        }
    ...
    for (index_t row = 0; row < row_count; ++row) {
      ...
      for (k = 0; k < rank; ++k) {
        for (l = 0; l < rank; ++l) {
          ...
          if (k == l) {
            B[part_id][pos_B] = gsptucker::constants::kLambda;
          }
          ++pos_B;
        }
