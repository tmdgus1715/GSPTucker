...
#include "gsptucker/baseline/update_nontiling.cuh"
...
void TuckerDecomposition(TensorType* input_tensor, int tucker_rank,
  ...
  matrix_t** B =
      host_agent->Allocate<matrix_t*>(max_partition_dim);
  matrix_t** C = host_agent->Allocate<matrix_t*>(max_partition_dim);
  ...
  for (index_t part = 0; part < max_partition_dim; ++part) {
    B[part] = host_agent->Allocate<matrix_t>(max_block_dim * tucker_rank *
    tucker_rank); C[part] = host_agent->Allocate<matrix_t>(max_block_dim *
    tucker_rank);
  }
